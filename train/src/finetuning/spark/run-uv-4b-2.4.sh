...
# Start monitoring
stdbuf -o0 vmstat -t 1 > "logs-4b/${JOB_NAME}-vmstat-${uid}.txt" &
VMSTAT_PID=$!
stdbuf -o0 nvidia-smi --query-gpu=timestamp,index,gpu_name,utilization.gpu,memory.used,memory.total,power.draw,temperature.gpu --format=csv -l 1 > "logs-4b/${JOB_NAME}-dmon-${uid}.csv" &
NVIDIA_PID=$!
