...
#SBATCH --mem 0
#SBATCH --job-name gemma3-base
#SBATCH --output gemma3-base.log
...
CUDA_VISIBLE_DEVICES=0,1 ./scripts/isambard-ai/serve_qwen_with_tools.sh > serve_qwen_with_tools_out.log 2>&1 &
CUDA_VISIBLE_DEVICES=2,3 ./scripts/isambard-ai/serve_t0.sh google/gemma-3-4b-it > serve_t0_1_gemma3-out.log 2>&1 &
...
t0-1 evaluate-rag ./data/synthetic_queries/5147cd8_gpt-4o_1000_synthetic_queries.jsonl --k 5 --db-choice chroma --llm-provider openai_completion --llm-model-name google/gemma-3-4b-it --budget-forcing --budget-forcing-kwargs '{"max_tokens_thinking": 256, "num_stop_skips": 3}' --extra-body '{"max_tokens": 256}' --prompt-template-path ./templates/rag_evaluation_prompt_deepseek_r1.txt --system-prompt-path ./templates/rag_evaluation_system_prompt_deepseek_r1.txt --output-file ./evaluate-rag-gemma3-k5-32B-thinking256-k5-chroma.jsonl --conditions-file ./data/nhs-conditions/v4/qwen_summarised_conditions.jsonl --persist-directory ./v4-summarised-db --local-file-store ./v4-summarised-lfs
